...
(*Get all dominators of the given node.*)
...
(*Get the loop-head from the given SCC.*)
...
(*Get the loop-break from the given loop-head and loop-tail.*)
...
= fun g tail head ->
	...
	let rec proceed_succs node =
		...
		| _ -> raise (Failure "Unroller.find_loop_break: fatal")
	in proceed_succs head

(*Get loop-tail, loop-head, and loop-break.*)
...
= fun cfg scc ->
	...
	let tail = 
		...
		with _ -> raise (Failure "Unroller.get_loop_nodes: fatal") in
	let loop_break = find_loop_break cfg tail loop_head in
	(tail, loop_head, loop_break)

(*Connect two nodes with cmd, adding to the cfg if not in the cfg.*)
...
(*Connect original and copied SCC's.*)
...
(*Unroll loop, given SCC.*)
let unroll_loop : IntraCfg.t -> loop_nodes -> scc -> IntraCfg.t
=fun cfg loop_nodes scc ->
	let (looptail, loophead, loopbreak) = loop_nodes in
...
let rec unroll_cfg_1 : IntraCfg.t -> scc list -> IntraCfg.t 
=fun cfg sccs ->
	if cfg.fd.svar.vname = "_G_" then cfg
			...
			if not (List.exists (fun scc -> List.length scc > 1) sccs) then (cfg)
					...
					let cfg' = unroll_loop cfg loop_nodes one_scc_longer_than1 in	(*unroll*)
					let cfg' = compute_scc cfg' |> compute_dom in			(*recompute SCC and Dom*)
					let sccs' = cfg'.scc_list in
					unroll_cfg_1 cfg' sccs'
			)
	)

(*********** k=0 unrolling ***********)
let edge_control : IntraCfg.t -> loop_nodes ->  IntraCfg.t
= fun cfg (tail, head, break) ->
	remove_edge tail head cfg
	|> add_edge tail break

let rec unroll_cfg_0 : IntraCfg.t -> IntraCfg.t
= fun cfg ->
	let sccs = cfg.scc_list in
	...
	then
			...
			let updated = List.fold_left (fun cfg scc ->
					if List.length scc < 2 then cfg 
							...
							edge_control cfg loop_nodes
				) cfg sccs in 
			unroll_cfg_0 updated
	else
			cfg
(**************************************)
...
= fun global ->
	...
	let unrolled_cfgs = BatMap.map (fun cfg -> 
		...
		unroll_cfg_1 cfg (cfg.scc_list)) cfgs in
